...
| `grep "Failed password"` |Failed passwordがあるディレクトリを探す |正規化 | |
| `awk '{print $(NF-3)}'` |最後から3列目を表示 |集約 | |
| `sort \| uniq -c \| sort -rn` |カウントと順番を整理する |集約 | |
| 同一IPが触ったユニークユーザー数を数える |不正IPを検知する |アラート | |
| `journalctl --since` で時間窓を切る |いつからのアクセスかを見る |収集 | |
...
Q1. SIEMに届くログは、誰が書いているか。書かなかった場合どうなるか。　自動検知がログ書いてる？検知できない
Q2. auth.log にも journald にも残らないホスト内の挙動を3つ挙げる。　　enginx停止、ディスク枯渇、cpu逼迫
